...
import_date = sys.argv[1] if len(sys.argv)>1 else ''
...
def import_table(import_date, conn):
    ...
    try:
        for row in query_job:  # API request - fetches results
            recno += 1
            if recno % 1000 == 0:
                print('Loaded %i records from BigQuery %s' % (recno, import_date))
        ...
        print('BigQuery table %s was not found' % import_date)
        return False
    finally:
        print('Loaded %i records from BigQuery %s (imported:%i, duplicates:%i, errors:%i)' % (recno, import_date, success, duplicates, errors))


if import_date:
    import_table(import_date, conn)
